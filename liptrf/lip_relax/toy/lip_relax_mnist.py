...
def main():
    ...
    model = Net(power_iter=args.power_iter, lmbda=1, lc_gamma=args.lc_gamma, lc_alpha=args.lc_alpha, lr=args.lr, eta=args.eta).cuda()
    ...
    for layer in layers:
        if not layer.prox_done:
            layer.weight = nn.Parameter(layer.weight_t.clone().detach())

    test(args, model, device, test_loader, criterion)
    evaluate(test_loader, model, 1.58, 10, args, None, u_test=None, save_u=False)  
    evaluate_pgd(test_loader, model, epsilon=1.58, niter=10, alpha=1.58/4)
