...
# inp = torch.randn(32, 8, 28, 28).cuda()
# out = inp * 2
...
# model = Conv2dX(8, 8, 3, padding=1, lmbda=1, lc_gamma=0.01, lc_alpha=0.01, lr=1.2, eta=1e-5).cuda()
# optim = torch.optim.SGD(model.parameters(), lr=0.1)
...
# model.weight = nn.Parameter(model.weight_t.reshape(model.weight.shape))
# pred = model(inp)
# loss = crit(pred, out)
