...
def main():
    ...
    model = ViT(image_size=32, patch_size=4, num_classes=10, channels=3,
        dim=512, depth=args.layers, heads=8, mlp_ratio=4, attention_type=args.attention_type, 
    ...
    if args.task == 'train':
        if not args.relax:
            weight_path = os.path.join(args.weight_path, f"{args.model}_cifar10_seed-{args.seed}_layers-{args.layers}")
        else:
            weight_path = os.path.join(args.weight_path, f"{args.model}_cifar10_seed-{args.seed}_layers-{args.layers}_relax-{args.lmbda}_warmup-{args.warmup}")
