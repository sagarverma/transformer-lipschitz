...
def train(loader, model, opt, epsilon, kappa, epoch, log, 
    ...
    for i, (X,y,idx) in enumerate(loader):
        ...
        for ll in range(len(u_train)):
            if u_train[ll] != None:
                u_train_data.append(u_train[ll][idx,:].cuda().float())
            else:
        ...
        for ll in range(len(u_train)):
            if u_train_idx[ll] != None:
                u_train[ll][idx,:] = u_train_idx[ll].clone().detach().half().cpu()
        
        if args.gloro:
            loss = local_loss
        else:
            loss = kappa1*ce + (1-kappa1)*(local_loss + args.sparse*sparse_loss)
        ...
        if 'relux' in args.model:
            # project all threshold to be positive
            for ll in range(len(model.module)):
                layer = model.module[ll]
                if layer.__class__.__name__=='ReLU_x':
                    layer.threshold.data = torch.clamp(layer.threshold.data, 0.)
        
        reduced_ce = reduce_tensor(ce.data, args.world_size)
        reduced_err = reduce_tensor(err, args.world_size)
        reduced_local_loss = reduce_tensor(local_loss.data, args.world_size)
        reduced_local_err = reduce_tensor(local_err, args.world_size)
        reduced_sparse_loss = reduce_tensor(sparse_loss.data, args.world_size)

        # measure accuracy and record loss
        losses.update(reduced_ce.item(), X.size(0))
        errors.update(reduced_err.item(), X.size(0))
        robust_losses.update(reduced_local_loss.item(), X.size(0))
        robust_errors.update(reduced_local_err.item(), X.size(0))
        sparse_losses.update(reduced_sparse_loss.item(), X.size(0))
        
        # measure elapsed time
        batch_time.update(time.time()-end)
        end = time.time()
        if global_rank==0:
            print(epoch, i, local_loss.detach().item(), local_err.item(), ce.item(), err.item(), file=log)
        ...
        if verbose and (i==0 or i==len(loader)-1 or (i+1) % verbose == 0): 
            ...
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'SP {sploss.val:.3f} ({sploss.avg:.3f})//'.format(
                   epoch, i+1, len(loader), batch_time=batch_time, eps=epsilon1,
                   data_time=data_time, loss=losses, errors=errors, 
                   rloss = robust_losses, rerrors = robust_errors, sploss=sparse_losses), end=endline)
        log.flush()
    ...
    for i, (X,y,idx) in enumerate(loader):
        ...
        if save_u:
            ...
            for ll in range(len(u_test)):
                if u_test[ll] != None:
                    u_test_data.append(u_test[ll][idx,:].cuda().float())
                else:
        ...
        if save_u:
            for ll in range(len(u_test)):
                if u_test_idx[ll] != None:
                    u_test[ll][idx,:] = u_test_idx[ll].clone().detach().half().cpu()
        ...
        reduced_local_loss = reduce_tensor(local_loss.data, args.world_size)
        reduced_local_err = reduce_tensor(local_err, args.world_size)
        reduced_sparse_loss = reduce_tensor(sparse_loss, args.world_size)
        ...
        if global_rank==0:
            print(epoch, i, local_loss.item(), local_err.item(), ce.item(), err.item(), file=log)
...
def robust_loss(net_local, epsilon, X, y, u_list=None, u_data=None, 
    ...
    return robust_loss, robust_err, u_list, u_data, sparse_loss

class net_Local_Lip(nn.Module):
    def __init__(self, model, args):
        super(net_Local_Lip, self).__init__()
        # For distributed trianing
    ...
    def forward(self, x, epsilon=None, u_list=None, u_data=None, sniter=1):
        ...
        # Compute global Lipschitz bound for part of the layers to save memory
        global_lip_partial = [] 
        sparse_loss = 0.0
        ...
        for i, layer in enumerate(self.model.children()):
            if (i+1)==depth:
                ...
                for col in range(len(global_lip_partial)):
                    r_tight *= global_lip_partial[col]
                r_tight *= eps
                mu_prev = mu
                r_prev = r_tight
                ibp_mu_prev = ibp_mu
                ...
                return mu, mu_prev, r_prev, ibp_mu, ibp_mu_prev, ibp_r_prev, layer.weight, u_list, self.u_data, sparse_loss
            
            # Box constrained propagation
            if isinstance(layer, nn.Conv2d):
                mu, r, p, ibp_mu, ibp_r = self.bcp_conv2d(layer, mu, r, ibp_mu, ibp_r, i)
                if 'relux' in args.model:
                    sparse_loss += self.bcp_mask(ibp_mu, ibp_r, i, self.model[i+1])
            elif isinstance(layer, nn.Linear):
                mu, r, p, ibp_mu, ibp_r = self.bcp_linear(layer, mu, r, ibp_mu, ibp_r, i)
                if 'relux' in args.model:
                    sparse_loss += self.bcp_mask(ibp_mu, ibp_r, i, self.model[i+1])
                ...
                sparse_loss += self.bcp_mask_maxmin(ibp_mu, ibp_r, i, layer)
            ...
            # record global Lip bound if u_data is not saved
            if len(self.u_data) > 0:
                if (self.u_data[i] == None) and (isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear)):
                    global_lip_partial.append(p)
                
    def local_linear(self):
        EPS = 1e-12
        tol = 1e-3
        depth = len(list(self.model.children())) # number of main layers
        # save local Lipschitz bound at each local layer, initialized as -1.0
        specnorms = []
        
        # compute local Lipschitz bound until the penultimate layer
        for i in range(depth-1):
            layer = self.model[i]
            # If having saved u, pass for non-conv and non-linear layers and layers without saved u
            if (len(self.u_data) > 0) and not ((isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear)) and (self.u_data[i] != None)):
                pass
            elif (isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear)):
            # if (isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear)) and (self.u_data[i] != None):
                W = layer.weight
                if len(self.u_data) > 0:
                ...
                for itr in range(self.sniter):
                    ...
                    if isinstance(layer, nn.Conv2d):
                        ...
                        #  When the output size differs from the expected one.
    ...
    def bcp_mask(self, ibp_mu, ibp_r, i, relux):
        ...
        # push values to be close to threshold_0 or threshold
        sparse_loss = torch.mean(F.relu(ibp_ub)) + self.args.sp_thres * torch.mean(F.relu(-(ibp_lb-relux.threshold)))
    ...
    def bcp_mask_maxmin(self, ibp_mu, ibp_r, i, layer):
        ...
        # encourage sparsity
        sparse_loss = torch.mean(F.relu(layer.max - maxlb)) + torch.mean(F.relu(minub - layer.min))
