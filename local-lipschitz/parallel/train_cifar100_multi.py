...
if __name__ == "__main__":
    args = utils.argparser(data='cifar100')
    print(datetime.now())
    print(args)
    ...
    for t in range(args.epochs): 
        ...
        if device_count == 1 or global_rank == 0:
            if t>=args.warmup+len(eps_schedule):    
                if err < best_err and args.save: 
                    ...
                    torch.save({
                        ...
                        }, f"{args.weight_path}/LocalLip_CIFAR100_{args.model}.pt")
            ...
            torch.save({ 
                ...
                'epoch' : t,
                'optimizer' : opt.state_dict()
                }, f"{args.weight_path}/LocalLip_CIFAR100_{args.model}_checkpoint.pt")  

    args.print = True
    torch.cuda.synchronize()
    
    model_eval = utils.select_model(args.data, args.model, args.init)
    aa = torch.load(f"{args.weight_path}/LocalLip_CIFAR100_{args.model}.pt", map_location="cpu")
